...
static __always_inline void check_canary(canary_t *can) {
	...
	if (unlikely(is_unset)) {
		...
		if (can->cpu_id != bpf_get_smp_processor_id() ||
		    can->magic_num != CANARY) {
...
static __always_inline event *reserve_event_slot() {
	...
	event_array_md *md =
	    (event_array_md *)bpf_map_lookup_elem(&events, &md_key);
	__u32 next_head;
	__u32 real_idx;
	event *evt;
	...
	if (ASSERTS) {
		...
		evt->event = 0;
		evt->ktime = 0;
		evt->magic_num[0] = CANARY;
	}
	return evt;
}

static __always_inline void commit_event(event *evt) {
	__u32 cpuid = bpf_get_smp_processor_id();
	__u32 map_len = (EVENT_BUFFER_SLOTS / NUM_CORES);
	__u32 md_key = EVENT_BUFFER_SLOTS + cpuid;
	event_array_md *md =
	...
	if (ASSERTS) {
		...
		if (next_head == md->tail)
			bpf_printk("Tried to commit a buffer slot while the ring buffer "
			           "was full\n");
