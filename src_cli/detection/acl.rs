...
pub struct ProfAccessRules {
    prote_files: HashMap<Pattern, AccessType>,
...
impl ProfAccessRules {
    ...
    fn insert_rule(&mut self, protectee: &str, mode: &str) -> Result<()> {
        ...
        self.prote_files.insert(Pattern::new(file)?, access_type);
        Ok(())
    }
...
impl fmt::Debug for ProfAccessRules {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut file_rules = self
            .prote_files
            .iter()
            .map(|(pattern, access)| (pattern.as_str().to_string(), *access))
            .collect::<Vec<_>>();

        file_rules.sort_by(|a, b| a.0.cmp(&b.0));

        f.debug_struct("ProfAccessRules")
            .field("files", &file_rules)
            .field("syscalls", &self.prote_syscalls)
            .finish()
    }
}
